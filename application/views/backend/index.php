...
<body>
            ...
            <div class="navbar-default sidebar" role="navigation">
                <div class="sidebar-nav navbar-collapse">
                    <ul class="nav" id="side-menu">
                        ...
                        <li>
                          <a href="#"><i class="fa fa-archive fa-fw"></i> Data Komik <span class="fa arrow"></span></a></a>
                            <ul class="nav nav-second-level">
                              <li>
                                <a href="<?php echo base_url('admin_system/komik') ?>"><i class="fa fa-eye fa-fw"></i> Data Komik</a>
                              </li>
                              <li>
                                <a href="<?php echo base_url('admin_system/komik_form') ?>"><i class="fa fa-plus fa-fw"></i> Data Komik</a>
                              </li>
                            </ul>
                        </li>
                        <li>
                        ...
                        <li>
                          <a href="#"><i class="fa fa-flag fa-fw"></i> Generate Laporan<span class="fa arrow"></span></a></a>
                            <ul class="nav nav-second-level">
                              <li>
                                <a href="<?php echo base_url('admin_system/laporan_pinjam') ?>"><i class="fa fa-flag-checkered fa-fw"></i> Komik Yang Dipinjam</a>
                              </li>
                              <!-- <li>
    ...
    <!-- WARNING VALIDATE SCRIPT  -->
    <script type="text/javascript">
      var jvalidate = $("#komik_form").validate({
        ignore: [],
        rules: {
          name: {
            required: true
        ...
        submitHandler: function(form) {
          var target = $(form).attr('action');
          $('#komik_form .alert-warning').removeClass('hidden');
          $('#komik_form .alert-success').addClass('hidden');
          $('#komik_form .alert-danger').addClass('hidden');
          $.ajax({
            url : target,
            type : 'POST',
            dataType : 'json',
            data : $(form).serialize(),
            success : function(response){
              $('#komik_form .alert-warning').addClass('hidden');
              if(response.status == 'ok'){
                $('#komik_form .alert-success').removeClass('hidden').children('span').text(response.msg);
                window.location.href = response.redirect;
              }
              else
                $('#komik_form .alert-danger').removeClass('hidden').children('span').text(response.msg);
            },
            error : function(jqXHR, textStatus, errorThrown) {
              alert(textStatus, errorThrown);
            }
